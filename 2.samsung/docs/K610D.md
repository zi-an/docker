...
```
sudo echo '{"registry-mirrors":["https://dockerhub.icu"]}' | tee -a /etc/docker/daemon.json  
echo "alias nginx='docker exec -it nginx bash'" >> ~/.bashrc
echo "alias test='docker exec -it test bash'" >> ~/.bashrc
echo "alias xunlei='docker exec -it xunlei bash'" >> ~/.bashrc 
```
